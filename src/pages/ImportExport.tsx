...
export default function ImportExport() {
  ...
  const startExportWithStateName = (exportType: PendingExport) => {
    setPendingExport(exportType);
    setStateNameDialogOpen(true);
  };

  const handleStateNameConfirm = async (stateName: string) => {
  ...
  const handleExportZip = async (stateName: string) => {
    setExporting('zip');
    setError(null);
    try {
  ...
  const handleExportPdf = async (stateName: string) => {
    setExporting('pdf');
    setError(null);
    try {
    ...
    <Container maxWidth="xl" sx={{ py: 4 }}>
      ...
      <Paper sx={{ p: 2, mb: 3 }}>
        <Grid container spacing={3}>
          <Grid size={{ xs: 6, sm: 3 }}>
            <Typography variant="overline" color="text.secondary">Total Capabilities</Typography>
            <Typography variant="h5">{statusCounts.total}</Typography>
          </Grid>
          <Grid size={{ xs: 6, sm: 3 }}>
            <Typography variant="overline" color="text.secondary">Finalized</Typography>
            <Typography variant="h5" color="success.main">{statusCounts.finalized}</Typography>
          </Grid>
          <Grid size={{ xs: 6, sm: 3 }}>
            <Typography variant="overline" color="text.secondary">In Progress</Typography>
            <Typography variant="h5" color="warning.main">{statusCounts.inProgress}</Typography>
          </Grid>
          <Grid size={{ xs: 6, sm: 3 }}>
